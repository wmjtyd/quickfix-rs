...
auto ApplicationCCApi::cancel_open_orders(const std::string &symbol) const -> void {
  this->ccapiwrapper.Request(CCAPI_EXECUTION_CANCEL_OPEN_ORDERS, symbol); // symbol = "BTCUSDT"
}
...
auto ApplicationCCApi::get_open_orders(const std::string &symbol) const -> void {
  this->ccapiwrapper.Request(CCAPI_EXECUTION_CANCEL_ORDER, symbol); // symbol = "BTCUSDT"
}
